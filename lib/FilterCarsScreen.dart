...
class _FilterCarsScreenState extends State<FilterCarsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.white,
        title: const Center(child: Text("Filter Cars")),
      ),
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            GestureDetector(
              onTap: () {
                List<Car> filteredCars = widget.allCars.where((car) {
                  bool matchesPrice =
                  ...
                  bool matchesFeatures = selectedFeatures.isEmpty ||
                      selectedFeatures.every((feature) =>
                          car.transmission.toLowerCase().contains(feature.toLowerCase()));

                  bool matchesSearch = searchText.isEmpty ||
                      car.name.toLowerCase().contains(searchText) ||
                      car.type.toLowerCase().contains(searchText);

                  return matchesPrice && matchesType && matchesFeatures && matchesSearch;
                }).toList();

                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => FilteredResultScreen(filteredCars: filteredCars),
                  ),
                );
              },
